            ...
            for(size_t r=0; r < pyramid[level-1].rows; r+=2)
                for(size_t c=0; c < nCols; c+=2)
                {
                    ...
                    if(nValidPixels > 0)
                    {
                        ...
                        _z_sub[pixel_sub] = avDepth / nValidPixels;
                    }
                }
        }
        else
...
double RegisterDense::errorDense( const int &pyramidLevel, const Eigen::Matrix4f poseGuess, costFuncType method )
{
    ...
    size_t nValidPhotoPts = 0;
    size_t nValidDepthPts = 0;
    ...
    warp_pixels.resize( pts_src_transformed.rows() );
    warp_img.resize( pts_src_transformed.rows(), 2 );
    residualsPhoto = Eigen::VectorXf::Zero(imgSize);
    residualsDepth = Eigen::VectorXf::Zero(imgSize);
    stdDevError_inv = Eigen::VectorXf::Zero(imgSize);
    wEstimPhoto = Eigen::VectorXf::Zero(imgSize);
    wEstimDepth = Eigen::VectorXf::Zero(imgSize);
    validPixelsPhoto = Eigen::VectorXi::Zero(imgSize);
    validPixelsDepth = Eigen::VectorXi::Zero(imgSize);

//    _residualsPhoto = Eigen::VectorXf::Zero(imgSize);
//    _residualsDepth = Eigen::VectorXf::Zero(imgSize);
//    _stdDevError_inv = Eigen::VectorXf::Zero(imgSize);
//    _wEstimPhoto = Eigen::VectorXf::Zero(imgSize);
//    _wEstimDepth = Eigen::VectorXf::Zero(imgSize);
//    _validPixelsPhoto = Eigen::VectorXi::Zero(imgSize);
//    _validPixelsDepth = Eigen::VectorXi::Zero(imgSize);
...
//        {
//            if( validPixels(i) ) //Compute the jacobian only for the valid points
//            {
//                Eigen::Vector3f xyz = rotation*LUT_xyz_source[vSalientPixels[pyramidLevel][i]] + translation;

//                //Project the 3D point to the 2D plane
//                float inv_transformedPz = 1.0/xyz(2);
//                float transformed_r,transformed_c; // 2D coordinates of the transformed pixel(r,c) of frame 1
//                transformed_c = (xyz(0) * fx)*inv_transformedPz + ox; //transformed x (2D)
//                transformed_r = (xyz(1) * fy)*inv_transformedPz + oy; //transformed y (2D)
...
//                        float pixel_trg = grayTrgPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int); //Intensity value of the pixel(r,c) of the warped target (reference) frame
//                        float weightedError = (pixel_trg - pixel_src)*stdDevPhoto_inv;
...
//                    {
//                        float depth_trg = depthTrgPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int);
//                        if(std::isfinite(depth_trg)) // Make sure this point has depth (not a NaN)
//                        {
//                            //Obtain the depth values that will be used to the compute the depth residual
//                            float depth1 = xyz(2);
//                            float stdDevError = std::max (stdDevDepth*(depth1*depth1+depth_trg*depth_trg), 2*stdDevDepth);
//                            float weightedError = (depth_trg - depth1)/stdDevError;
//                            float weight_estim = weightMEstimator(weightedError);
                                    ...
                                    costFuncType method )
{
    ...
    //                        jacobianWarpRt(0,2)=-fx*xyz(0)*inv_transformedPz*inv_transformedPz;
    //                        jacobianWarpRt(1,2)=-fy*xyz(1)*inv_transformedPz*inv_transformedPz;

    //                        //Derivative with respect to \w_x
    //                        jacobianWarpRt(0,3)=-fx*rotatedPoint3D(1)*xyz(0)*inv_transformedPz*inv_transformedPz;
    //                        jacobianWarpRt(1,3)=-fy*(rotatedPoint3D(2)+rotatedPoint3D(1)*xyz(1)*inv_transformedPz)*inv_transformedPz;

    //                        //Derivative with respect to \w_y
    //                        jacobianWarpRt(0,4)= fx*(rotatedPoint3D(2)+rotatedPoint3D(0)*xyz(0)*inv_transformedPz)*inv_transformedPz;
    //                        jacobianWarpRt(1,4)= fy*rotatedPoint3D(0)*xyz(1)*inv_transformedPz*inv_transformedPz;
    ...
    //                        float pixel_src, pixel_trg, depth1, depth_trg;
    ...
    //                            pixel_trg = grayTrgPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int); //Intensity value of the pixel(r,c) of the warped target (reference) frame
    ////                        cout << "pixel_src " << pixel_src << " pixel_trg " << pixel_trg << endl;
    //                            float weightedError = (pixel_trg - pixel_src)*stdDevPhoto_inv;
    //                            float weight_estim = weightMEstimator(weightedError);
    ...
    //                        {
    //                            //Obtain the depth values that will be used to the compute the depth residual
    //                            depth_trg = depthTrgPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int);
    //                            if(std::isfinite(depth_trg)) // Make sure this point has depth (not a NaN)
    //                            {
    //                                depth1 = depthSrcPyr[pyramidLevel].at<float>(r,c); // Depth value of the pixel(r,c) of the warped frame 1 (target)
    //                                float weightedError = depth_trg - depth1;
        ...
        for(size_t i=0; i < pts_src_transformed.rows(); i++)
            ...
            if( validPixels(i) ) //Compute the jacobian only for the valid points
            {
                        ...
                        (transformed_c_int>=0 && transformed_c_int < nCols) )
                {
                    ...
                    jacobianWarpRt(0,2)=-fx*xyz(0)*inv_transformedPz_2;
                    jacobianWarpRt(1,2)=-fy*xyz(1)*inv_transformedPz_2;

                    //Derivative with respect to \w_x
                    jacobianWarpRt(0,3)=-fx*xyz(1)*xyz(0)*inv_transformedPz_2;
                    jacobianWarpRt(1,3)=-fy*(1+xyz(1)*xyz(1)*inv_transformedPz_2);

                    //Derivative with respect to \w_y
                    jacobianWarpRt(0,4)= fx*(1+xyz(0)*xyz(0)*inv_transformedPz_2);
                    jacobianWarpRt(1,4)= fy*xyz(0)*xyz(1)*inv_transformedPz_2;

                    //Derivative with respect to \w_z
                    jacobianWarpRt(0,5)=-fx*xyz(1)*inv_transformedPz;
                    jacobianWarpRt(1,5)= fy*xyz(0)*inv_transformedPz;
...
//    //                                //float weight_estim = weightMEstimator(weightedError,stdDevDepth)*stdDevPhoto_inv;
//    //                                //float stdDevError = stdDevDepth*depth1;
//    //                                float stdDevError = std::max (stdDevDepth*(xyz(2)*xyz(2)+depth_trg*depth_trg), 2*stdDevDepth);
...
//                    float depth_trg = depthTrgPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int);
//                    float weightedError = depth_trg - inv_transformedPz;
...
#pragma omp parallel for reduction (+:error2,numValidPts) // numValidPtsPhoto, numValidPtsDepth
        ...
        for(size_t i=0; i < pts_src_transformed.rows(); i++)
        {
            if( validPixels(i) ) //Compute the jacobian only for the valid points
            {
                ...
                float phi_trg = asin(xyz(1)*dist_inv);
                float theta_trg = atan2(xyz(0),xyz(2));
                //int transformed_r_int = half_height + int(round(phi_trg*pixel_angle_inv));
                size_t transformed_r_int = int(round((phi_trg-phi_start)*pixel_angle_inv));
                ...
                {
                    size_t transformed_c_int = half_width + int(round(theta_trg*pixel_angle_inv)) % half_width;
                    size_t warped_i = transformed_r_int * nCols + transformed_c_int;
                    warp_pixels(i) = warped_i;
...
//                        float pixel_trg = grayTrgPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        ...
                        residualsPhoto(i) = diff * stdDevPhoto_inv;
                        wEstimPhoto(i) = weightMEstimator(residualsPhoto(i)); // Apply M-estimator weighting // The weight computed by an M-estimator
                        error2 += wEstimPhoto(i) * residualsPhoto(i) * residualsPhoto(i);

//                        _validPixelsPhoto(numValidPtsPhoto) = i;
//                        _residualsPhoto(numValidPtsPhoto) = (_grayTrgPyr[warped_i] - _graySrcPyr[numValidPtsPhoto]) * stdDevPhoto_inv;
//                        _wEstimPhoto(numValidPtsPhoto) = weightMEstimator(_residualsPhoto(numValidPtsPhoto)); // Apply M-estimator weighting // The weight computed by an M-estimator
//                        error2 += _wEstimPhoto(numValidPtsPhoto) * _residualsPhoto(numValidPtsPhoto) * _residualsPhoto(numValidPtsPhoto);

                         v_AD_intensity[numValidPtsPhoto] = fabs(diff);
                    ...
                    {
                        //float depth_trg = depthTrgPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int);
                        float depth_trg = _depthTrgPyr[warped_i];
                        if(depth_trg> 0) // if(std::isfinite(depth_trg)) // Make sure this point has depth (not a NaN)
                        {
                            if( fabs(_depthTrgGradXPyr[warped_i]) < thresSaliencyIntensity && fabs(_depthTrgGradYPyr[warped_i]) < thresSaliencyDepth)
                                continue;

                            validPixelsDepth(i) = 1;

                            //Obtain the depth values that will be used to the compute the depth residual
                            stdDevError_inv(i) = 1 / std::max (stdDevDepth*(dist*dist+depth_trg*depth_trg), 2*stdDevDepth);
                            residualsDepth(i) = (depth_trg - dist) * stdDevError_inv(i);
                            wEstimDepth(i) = weightMEstimator(residualsDepth(i)); // Apply M-estimator weighting // The weight computed by an M-estimator
                            error2 += wEstimDepth(i) * residualsDepth(i) * residualsDepth(i);
                            // std::cout << i << " error2 " << error2 << " wDepthError " << weightedError << " weight_estim " << weight_estim << " diff " << diff << " " << stdDevError << std::endl;

//                            _validPixelsDepth(numValidPtsDepth) = i;
//                            _stdDevError_inv(numValidPtsDepth) = 1 / std::max (stdDevDepth*(dist*dist+depth_trg*depth_trg), 2*stdDevDepth);
//                            _residualsDepth(numValidPtsDepth) = (_grayTrgPyr[warped_i] - _graySrcPyr[numValidPtsDepth]) * stdDevError_inv(numValidPtsDepth);
//                            _wEstimDepth(numValidPtsDepth) = weightMEstimator(_residualsDepth(numValidPtsDepth)); // Apply M-estimator weighting // The weight computed by an M-estimator
//                            error2 += _wEstimDepth(numValidPtsDepth) * _residualsDepth(numValidPtsDepth) * _residualsDepth(numValidPtsDepth);
...
#pragma omp parallel for reduction (+:error2,numValidPts) // numValidPtsPhoto, numValidPtsDepth
        ...
        for(size_t i=0; i < pts_src_transformed.rows(); i++)
        {
            ...
            if( validPixels(i) ) //Compute the jacobian only for the valid points
            {
                ...
                float phi_trg = asin(xyz(1)*dist_inv);
                float theta_trg = atan2(xyz(0),xyz(2));
                float transformed_r = (phi_trg-phi_start)*pixel_angle_inv;
                ...
                if( transformed_r_int>=0 && transformed_r_int < nRows) // && transformed_c_int < nCols )
                {
                    float transformed_c = half_width + theta_trg*pixel_angle_inv; if(transformed_c > half_width) transformed_c -= half_width;
                    size_t transformed_c_int = int(round(transformed_c)) % half_width;
                    size_t warped_i = transformed_r_int * nCols + transformed_c_int;
                    warp_pixels(i) = warped_i;
                    warp_img(i,0) = transformed_r;
                    warp_img(i,1) = transformed_c;
                    // std::cout << "Pixel transform_ " << i/nCols << " " << i%nCols << " " << transformed_r_int << " " << transformed_c_int << " " << nRows << "x" << nCols << " numValidPts " << numValidPts << endl;
                    // assert(transformed_c_int >= 0 && transformed_c_int < nCols);
                    ...
                    if(method == PHOTO_CONSISTENCY || method == PHOTO_DEPTH)
                    {
                        ...
                        float pixel_trg = bilinearInterp( grayTrgPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) ); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        residualsPhoto(i) = diff * stdDevPhoto_inv;
                        wEstimPhoto(i) = weightMEstimator(residualsPhoto(i)); // Apply M-estimator weighting // The weight computed by an M-estimator
                        error2 += wEstimPhoto(i) * residualsPhoto(i) * residualsPhoto(i);
                    ...
                    if(method == DEPTH_CONSISTENCY || method == PHOTO_DEPTH)
                    {
                        float depth_trg = bilinearInterp_depth( grayTrgPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) ); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        if(std::isfinite(depth_trg)) // Make sure this point has depth (not a NaN)
                        {
...
//    _validPixelsPhoto.resize(numValidPtsPhoto);
//    _residualsPhoto.resize(numValidPtsPhoto);
//    _wEstimPhoto.resize(numValidPtsPhoto);

//    _validPixelsDepth.resize(numValidPtsPhoto);
//    _stdDevError_inv.resize(numValidPtsPhoto);
//    _residualsDepth.resize(numValidPtsPhoto);
//    _wEstimDepth.resize(numValidPtsPhoto);
    ...
    std::cout << " stdDevPhoto      " << stdDevPhoto << std::endl;
...
#pragma omp parallel for reduction(+:numVisiblePixels)
        ...
        for(size_t i=0; i < pts_src_transformed.rows(); i++)
        {
            ...
            {
                Eigen::Vector3f xyz = pts_src_transformed.block(i,0,1,3).transpose();
                // cout << "3D pts " << xyz.transpose() << " trnasformed " << xyz.transpose() << endl;

                //Projected 3D point to the S2 sphere
                float dist = xyz.norm();

                Eigen::Matrix<float,2,6> jacobianWarpRt;
                computeJacobian26_wT(xyz, dist, pixel_angle_inv, jacobianWarpRt);

                if(method == PHOTO_CONSISTENCY || method == PHOTO_DEPTH)
                {
                    if(visualizeIterations)
                        warped_source_grayImage.at<float>(warp_pixels(i)) = graySrcPyr[pyramidLevel].at<float>(i);

                    Eigen::Matrix<float,1,2> target_imgGradient;
                    target_imgGradient(0,0) = _grayTrgGradXPyr[warp_pixels(i)];
                    target_imgGradient(0,1) = _grayTrgGradYPyr[warp_pixels(i)];

                    //Obtain the pixel values that will be used to compute the pixel residual
                    //Apply the chain rule to compound the image gradients with the projective+RigidTransform jacobians
                    float weight_estim_sqrt = sqrt(wEstimPhoto(i));
                    jacobiansPhoto.block(i,0,1,6).noalias() = ((weight_estim_sqrt * stdDevPhoto_inv) * target_imgGradient) * jacobianWarpRt;
                    residualsPhoto(i) *= weight_estim_sqrt;
                ...
                {
                    if(visualizeIterations)
                        warped_source_depthImage.at<float>(warp_pixels(i)) = dist;

                    Eigen::Matrix<float,1,2> target_depthGradient;
                    target_depthGradient(0,0) = _depthTrgGradXPyr[warp_pixels(i)];
                    target_depthGradient(0,1) = _depthTrgGradYPyr[warp_pixels(i)];
                    // std::cout << "target_depthGradient \n " << target_depthGradient << std::endl;

                    //Depth jacobian:
                    //Apply the chain rule to compound the depth gradients with the projective+RigidTransform jacobians
                    Eigen::Matrix<float,1,6> jacobian16_depthT = Eigen::Matrix<float,1,6>::Zero();
                    jacobian16_depthT.block(0,0,1,3) = (1 / dist) * xyz.transpose();
                    float weight_estim_sqrt = sqrt(wEstimDepth(i));
                    jacobiansDepth.block(i,0,1,6) = weight_estim_sqrt * stdDevError_inv(i) * (target_depthGradient*jacobianWarpRt - jacobian16_depthT);;
                    residualsDepth(i) *= weight_estim_sqrt;
                    // std::cout << "residualsDepth \n " << residualsDepth << std::endl;
...
#pragma omp parallel for reduction(+:numVisiblePixels)
        ...
        for(int i=0; i < pts_src_transformed.rows(); i++)
        {
                ...
                // if(min_depth_ < depth1 && depth1 < max_depth_) //Compute the jacobian only for the valid points
            {
                ...
                //Eigen::Vector3f xyz = rotation*LUT_xyz_source[i] + translation;
                Eigen::Vector3f xyz = pts_src_transformed.block(i,0,1,3).transpose();
//                Eigen::Vector3f xyz = pts_src_transformed.block(0,i,3,1);
                ...
                float transformed_c = half_width + theta_trg*pixel_angle_inv; if(transformed_c > half_width) transformed_c -= half_width;
                int transformed_r_int = round(transformed_r);
                int transformed_c_int = int(round(transformed_c)) % half_width;
                // std::cout << "Pixel transform " << i/nCols << " " << i%nCols << " " << transformed_r_int << " " << transformed_c_int << std::endl;
                // mrpt::system::pause();
                //Asign the intensity value to the warped image and compute the difference between the transformed
                //pixel of the source frame and the corresponding pixel of target frame. Compute the error function
...
//                    mrpt::system::pause();
                    ...
                    float pixel_src, pixel_trg, depth_trg;
                    float weightedErrorPhoto, weightedErrorDepth;
                    Eigen::Matrix<float,1,6> jacobianPhoto, jacobianDepth;
                    if(method == PHOTO_CONSISTENCY || method == PHOTO_DEPTH)
                    {
                        ...
                        Eigen::Matrix<float,1,2> target_imgGradient;
                        target_imgGradient(0,0) = bilinearInterp( grayTrgGradXPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) );
                        target_imgGradient(0,1) = bilinearInterp( grayTrgGradYPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) );
                        if(fabs(target_imgGradient(0,0)) < thresSaliencyIntensity && fabs(target_imgGradient(0,1)) < thresSaliencyIntensity)
                            continue;
                        ...
                        //pixel_src = graySrcPyr[pyramidLevel].at<float>(r,c); // Intensity value of the pixel(r,c) of source frame
                        pixel_src = graySrcPyr[pyramidLevel].at<float>(i); // Intensity value of the pixel(r,c) of source frame
                        pixel_trg = bilinearInterp( grayTrgPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) ); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        // std::cout << "pixel_src " << pixel_src << " pixel_trg " << pixel_trg << std::endl;
                        float photoDiff = pixel_trg - pixel_src;
...
//                            jacobiansPhoto.block(i,0,1,6) = wEstimPhoto[i] * target_imgGradient*jacobianWarpRt;
                    ...
                    {
                        //Obtain the depth values that will be used to the compute the depth residual
                        depth_trg = bilinearInterp_depth( grayTrgPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) ); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        if(std::isfinite(depth_trg)) // Make sure this point has depth (not a NaN)
                        {
                            if(visualizeIterations)
                            ...
                            Eigen::Matrix<float,1,2> target_depthGradient;
                            target_depthGradient(0,0) = bilinearInterp_depth( depthTrgGradXPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) );
                            target_depthGradient(0,1) = bilinearInterp_depth( depthTrgGradYPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) );
                            if(fabs(target_depthGradient(0,0)) < thresSaliencyDepth && fabs(target_depthGradient(0,1)) < thresSaliencyDepth)
                                continue;

                            float stdDevError_inv = 1 / std::max (stdDevDepth*(dist*dist+depth_trg*depth_trg), 2*stdDevDepth);
                            float depthDiff = depth_trg - dist;
                            ...
                            jacobianDepth = weight_estim * (target_depthGradient*jacobianWarpRt - jacobian16_depthT);
...
//    std::vector<float> v_AD_intensity( LUT_xyz_target.size() );
    ...
    transformPts3D_sse(LUT_xyz_target, poseGuess, pts_trg_transformed);

    warp_pixels.resize( pts_trg_transformed.rows() );
    warp_img.resize( pts_trg_transformed.rows(), 2 );
    residualsPhoto = Eigen::VectorXf::Zero(imgSize);
    residualsDepth = Eigen::VectorXf::Zero(imgSize);
    stdDevError_inv = Eigen::VectorXf::Zero(imgSize);
    wEstimPhoto = Eigen::VectorXf::Zero(imgSize);
            ...
            if( validPixels(i) ) //Compute the jacobian only for the valid points
            {
                ...
                if( transformed_r_int>=0 && transformed_r_int < nRows ) //&& transformed_c_int < nCols )
                {
                    ...
                    if(method == PHOTO_CONSISTENCY || method == PHOTO_DEPTH)
                    {
                        ...
                        float pixel_trg = graySrcPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        //float pixel_trg = bilinearInterp( graySrcPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) ); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        float diff = pixel_trg - pixel_src;
                        float weightedError = diff*stdDevPhoto_inv; // (pixel_trg - pixel_src)
...
//                        validPixelsPhoto(i) = 1;
                    ...
                    if(method == DEPTH_CONSISTENCY || method == PHOTO_DEPTH)
                    {
                        float depth_trg = depthSrcPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int);
                        //float depth_trg = bilinearInterp_depth( graySrcPyr[pyramidLevel], cv::Point2f(transformed_r, transformed_c) ); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        if(std::isfinite(depth_trg)) // Make sure this point has depth (not a NaN)
                        {
                            ...
                            float stdDevError = std::max (stdDevDepth*(dist*dist+depth_trg*depth_trg), 2*stdDevDepth);
                            float diff = depth_trg - dist;
...
#pragma omp parallel for reduction(+:numVisiblePixels)
        ...
        for(int i=0; i < LUT_xyz_target.size(); i++)
        {
                ...
                // if(min_depth_ < depth1 && depth1 < max_depth_) //Compute the jacobian only for the valid points
            {
                ...
                //Eigen::Vector3f xyz = rotation*point3D + translation;
                //Eigen::Vector3f xyz = pts_src_transformed.block(0,i,3,1);
                Eigen::Vector3f xyz = rotation_inv*LUT_xyz_target[i] + translation_inv;
                // cout << "3D pts " << point3D.transpose() << " trnasformed " << xyz.transpose() << endl;
                //Project the 3D point to the S2 sphere
                float dist = xyz.norm();
                float dist_inv = 1.f / dist;
                ...
                if( transformed_r_int>=0 && transformed_r_int < nRows) //&& transformed_c_int < nCols )
                {
                    ...
                    if(method == PHOTO_CONSISTENCY || method == PHOTO_DEPTH)
                    {
                        ...
                        //Obtain the pixel values that will be used to compute the pixel residual
                        //pixel_src = graySrcPyr[pyramidLevel].at<float>(r,c); // Intensity value of the pixel(r,c) of source frame
                        pixel_src = grayTrgPyr[pyramidLevel].at<float>(i); // Intensity value of the pixel(r,c) of source frame
                        pixel_trg = graySrcPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int); //Intensity value of the pixel(r,c) of the warped target (reference) frame
                        // std::cout << "pixel_src " << pixel_src << " pixel_trg " << pixel_trg << std::endl;
                        float photoDiff = pixel_trg - pixel_src;
                        float weight_estim = weightHuber(photoDiff*stdDevPhoto_inv);
                        weightedErrorPhoto = weight_estim * photoDiff * stdDevPhoto_inv;
...
//                            jacobiansPhoto.block(i,0,1,6) = wEstimPhoto[i] * target_imgGradient*jacobianWarpRt;
                    ...
                    {
                        //Obtain the depth values that will be used to the compute the depth residual
                        depth_trg = depthSrcPyr[pyramidLevel].at<float>(transformed_r_int,transformed_c_int);
                        if(std::isfinite(depth_trg)) // Make sure this point has depth (not a NaN)
                        {
                            if(visualizeIterations)
                                warped_source_depthImage.at<float>(transformed_r_int,transformed_c_int) = dist;
...
//                            if(nRows >20){
//                             std::cout << "jacobianDepthSrc " << jacobianDepthSrc << " inv " << jacobianDepthSrc*jacobianT36_inv << " weightedErrorDepth " << weightedErrorDepth << std::endl;
//                             mrpt::system::pause();}
...
//                float dist = xyz.norm();
//                float dist_inv = 1.f / dist;
//                float phi_trg = asin(xyz(0)*dist_inv);
//                float theta_trg = atan2(xyz(1),xyz(2))+PI;
...
//                            //Obtain the depth values that will be used to the compute the depth residual
//                            float stdDevError = std::max (stdDevDepth*(dist*dist+depth_trg*depth_trg), 2*stdDevDepth);
//                            float weightedError = (depth_trg - dist)/stdDevError;
//                            float weight_estim = weightMEstimator(weightedError);
//                            float weightedErrorDepth = weight_estim * weightedError;
//                            residualsDepth(ii) = weightedErrorDepth*weightedErrorDepth;
//                            ++nValidDepthPts;
...
//                            //Obtain the depth values that will be used to the compute the depth residual
//                            //float depth1 = depthSrcPyr[pyramidLevel].at<float>(r,c); // Depth value of the pixel(r,c) of the warped frame 1 (target)
//                            //float stdDevError = stdDevDepth*depth_trg;
//                            float stdDevError = std::max (stdDevDepth*(dist*dist+depth_trg*depth_trg), 2*stdDevDepth);
//                            float weight_estim = weightMEstimator(weightedError);
//                            float weightedErrorDepth = weight_estim * weightedError;
...
//        computePointsXYZ(depthSrcPyr[pyramidLevel], LUT_xyz_source);
        computeSphereXYZ_sse(depthSrcPyr[pyramidLevel], LUT_xyz_source);
...
void RegisterDense::alignFrames360_inv(const Eigen::Matrix4f pose_guess, costFuncType method , const int occlusion )
{
    ...
    for(int pyramidLevel = nPyrLevels-1; pyramidLevel >= 0; pyramidLevel--)
    {
        ...
        // Make LUT to store the values of the 3D points of the source sphere
        computeSphereXYZ_sse(depthTrgPyr[pyramidLevel], LUT_xyz_target);
...
void RegisterDense::alignFrames360_bidirectional(const Eigen::Matrix4f pose_guess, costFuncType method , const int occlusion )
{
    ...
    for(int pyramidLevel = nPyrLevels-1; pyramidLevel >= 0; pyramidLevel--)
    {
        ...
        //error = errorDense_sphere(pyramidLevel, pose_estim, method);
        if(occlusion == 0)
            error = errorDense_sphere_bidirectional(pyramidLevel, pose_estim, method);
        else
            assert(false);
        //            std::cout << "error  " << error << std::endl;
...
//            if((hessian + lambda*getDiagonalMatrix(hessian)).rank() != 6)
            ...
            double new_error;
            if(occlusion == 0)
                new_error = errorDense_sphere_bidirectional(pyramidLevel, pose_estim_temp, method);
            else
                assert(false);
...
//                //Project the 3D point to the 2D plane
//                double inv_transformedPz = 1.0/xyz(2);
...
//                    //Derivative with respect to z
//                    jacobianProj23(0,2)=-fx*xyz(0)*inv_transformedPz*inv_transformedPz;
//                    jacobianProj23(1,2)=-fy*xyz(1)*inv_transformedPz*inv_transformedPz;
...
void RegisterDense::computeSphereXYZ(const cv::Mat & depth_img, Eigen::MatrixXf & sphere_xyz)
{
...
/*! Compute the 3D points XYZ by multiplying the unit sphere by the spherical depth image. */
void RegisterDense::computeSphereXYZ_sse(const cv::Mat & depth_img, Eigen::MatrixXf & sphere_xyz)
{
    const size_t nRows = depth_img.rows;
    const size_t nCols = depth_img.cols;
    const size_t imgSize = nRows*nCols;
